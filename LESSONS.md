...
## Capturing real command output for docs

- **Lesson:** running `upload` (for a README demo, no `--gist-id` passed) didn't create a disposable gist — `resolve_gist_id`'s auto-discovery found the maintainer's own real, already-in-use snapshot gist (the only one tagged with this tool's description on that account) and silently added the demo file to it.
  - **Why:** the auto-discovery feature is deliberately convenient for the real cross-machine-sync use case (exactly the behavior wanted when a user runs it for real) — but that same convenience means "run it once to see what the output looks like" is never actually safe unless a disposable target is pinned explicitly. There is no "dry run" mode for `upload`.
  - **Avoid:** before running any command that talks to a real external account/service to capture output for documentation, check whether it has an auto-discovery/auto-reuse path and pin an explicit, disposable target (a throw-away `--gist-id`, a scratch identity) rather than trusting default/no-args behavior to stay local. When it does touch something real by accident, fix it immediately and visibly (here: `gh gist edit --remove` to restore the account to its prior state) rather than leaving it and hoping it doesn't matter.

## Reasoning errors

- **Wrong call**: asserted (twice, in two different README locations) that `diff` "is optional" / "can be skipped" in the two-machine sync flow.
  - **Based on**: the true, separate fact that `apply` determines what's missing by querying `claude plugin list` live rather than reading the diff table's printed content — which is real, but doesn't extend to "the `diff` command itself never needs to run."
  - **Why it was wrong**: `apply`'s CLI signature takes a `diff --out <file>` JSON file as its required first positional argument (`plugin_sync/cli.py`'s `apply_.add_argument("merged_file", ...)`, no default) — there is no code path where `apply` runs without that file existing, so the command that produces it cannot be skipped. Only *reading what it printed* is skippable. The two claims sound adjacent but aren't the same claim, and the wrong one was written first both times.
  - **Check first next time**: before writing "X is optional" about a step in a pipeline, check whether the *next* step's actual invocation (its required arguments, not just its typical use) depends on X's output — grep the argparse definition, not just the prose description of what the step is "for".
